...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    await sql`
      ...
      WHERE id=${params.id}
    `;
    await sql`DELETE FROM project_tags WHERE project_id=${params.id}`;
    if (p.tags?.length) {
      for (const tagId of p.tags) {
        await sql`INSERT INTO project_tags (project_id, tag_id) VALUES (${params.id}, ${tagId}) ON CONFLICT DO NOTHING`;
      }
    }
